...
//GAMEBOYS NOTE:
//THIS JAVA FILE IS ONLY FOR PLAYING THE MUSIC 
//AND ITS A SHARED LIBRARY ON THE INTERNET TO PLAY BIG SOUND FILES.
//ITS AN IMPROVED VERSION OF THE CLIP CLASS IN JAVA.
//ITS NEEDED TO PLAY THE SOUND FILE IN MAC
...
public class BigClip implements Clip, LineListener {
    ...
    public void update(LineEvent le) {
        logger.log(java.util.logging.Level.FINEST, "update: " + le );
    }

    @Override
    public void loop(int count) {
        logger.log(java.util.logging.Level.FINEST, "loop(" + count + ") - framePosition: " + framePosition);
        loopCount = count;
        countDown = count;
        active = true;
        inputStream.reset();
    ...
    @Override
    public void stop() {
        logger.log(java.util.logging.Level.FINEST, "BigClip.stop()");
        active = false;
    ...
    public void setFastForward(boolean fastForward) {
        logger.log(java.util.logging.Level.FINEST, "FastForward " + fastForward);
    ...
    public void setFastRewind(boolean fastRewind) {
        logger.log(java.util.logging.Level.FINEST, "FastRewind " + fastRewind);
        ...
        Runnable r = new Runnable() {
            public void run() {
                try {
                    ...
                    logger.log(java.util.logging.Level.FINEST, "loopCount " + loopCount );
                    while ((bytesRead = inputStream.read(data,0,data.length))
                        ...
                        framePosition -= 2*framesRead;
                            ...
                            logger.log(java.util.logging.Level.INFO, "totalBytes " + totalBytes);
                        } else {
                            newData = tempData;
                        }
    ...
    public byte[] reverseFrames(byte[] data) {
        ...
        for (int ii=0; ii<data.length/4; ii++) {
            ...
            if (ii<5 || ii>(data.length/4)-5) {
                logger.log(java.util.logging.Level.FINER, "From \t" + first + " \tlast " + last );
                logger.log(java.util.logging.Level.FINER, "To \t" + ((ii*4)+0) + " \tlast " + ((ii*4)+3) );
            }
        }
...
*/
    ...
    public byte[] getEveryNthFrame(byte[] data, int skip) {
        ...
        length = (length/4)*4;
        logger.log(java.util.logging.Level.FINEST, "length " + data.length + " \t" + length);
    ...
    public double getLargestSampleSize() {
        ...
        // audioData
        logger.log(java.util.logging.Level.FINEST, "Max signal level: " + (double)largest/(Math.pow(2, bitDepth-1)));
